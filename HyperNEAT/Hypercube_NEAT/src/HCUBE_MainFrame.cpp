...
namespace HCUBE
{
    ...
    void MainFrame::rerunAll( wxCommandEvent& event )
    {
        for(int generation = generationSpinner->GetMin(); generation <= generationSpinner->GetMax(); generation++) {
            int individual = 1;
            string title;
