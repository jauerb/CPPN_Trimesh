...
namespace HCUBE
{
    ...
    TicTacToeGameExperiment::TicTacToeGameExperiment(string _experimentName)
            :
    ...
    void TicTacToeGameExperiment::populateSubstrate(shared_ptr<const NEAT::GeneticIndividual> individual)
    {
    ...
    {
        generation->setUserData(shared_ptr<TicTacToeStats>(new TicTacToeStats()));
    }
    ...
    {
        shared_ptr<TicTacToeStats> ticTacToeStats = static_pointer_cast<TicTacToeStats>(generation->getUserData());

        if (individual->getUserData())
            (*ticTacToeStats) += *(static_pointer_cast<TicTacToeStats>(individual->getUserData()).get());
    }
}
