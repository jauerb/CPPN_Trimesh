...
namespace NEAT
{
    ...
    void Globals::assignParamID(GeneticParamGene *testParam)
    {
        testParam->setID(generateParamID());
    }
            ...
            paramCounter(0)
    {
        ...
        parameters.insert("NumLeavesWeight", 1.0);
            ...
            paramCounter(-1)
    {
        ...
        while (firstAttribute)
        {
            ...
            else if (iequals(firstAttribute->Name(),"ParamsCounter"))
            {
                paramCounter = firstAttribute->IntValue();
            }
            else
            {
    ...
    void Globals::dump(TiXmlElement *root)
    {
        ...
        root->SetAttribute("ParamCounter",paramCounter);
    ...
    double Globals::getParameterValue(const char *cname, double defaultValue)
    {
        if(parameters.hasKey(cname))
            return parameters.getDataRef(cname);
        else
            return defaultValue;
    }

    double Globals::getParameterValue(string name, double defaultValue)
    {
	...
	{
	    extraActivationUpdates = getParameterValue("ExtraActivationUpdates");
