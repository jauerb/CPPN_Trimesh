...
namespace NEAT
{
    ...
    void GeneticGeneration::dumpBest(TiXmlElement *generationElement,bool includeGenes)
    {
        ...
        {
            for (int a=0;a<nonDominatedIndividuals.size();a++)
            {
                TiXmlElement *individualElement = new TiXmlElement("Individual");
                nonDominatedIndividuals[a]->dump(individualElement,includeGenes);
                generationElement->LinkEndChild(individualElement);
            }
    ...
    void GeneticGeneration::dumpReproduced(TiXmlElement *generationElement,bool includeGenes)
    {
        setAttributes(generationElement);

        for (int a=0;a<(int)individuals.size();a++)
        {
            if (individuals[a]->getHasReproduced()) {
                TiXmlElement *individualElement = new TiXmlElement("Individual");
    ...
    void GeneticGeneration::sortByFitness()
    {
        ...
        stable_sort(individuals.begin(), individuals.end(), comparator);
